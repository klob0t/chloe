    ...
    return {
        ...
        sendMessage: async (content: string, model = 'openai-large') => {
            ...
            try {
                ...
                const response = await request(payload)
                console.log('[chat] sending payload to /api/completion', {
                    model,
                ...
                const assistantContentRaw = typeof response?.response === 'string' ? response.response : ''
                if (!assistantContentRaw || assistantContentRaw.trim().length === 0) {
                    console.warn('Assistant returned empty response', { payload, historyCount: serialisedHistory.length })
                }
        ...
        saveConversation: (title?: string) => {
            ...
            if (hasEmptyAssistant) {
                console.log('Skipping save: assistant message still empty')
                return
            }
